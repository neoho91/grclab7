...
try:
    from powermeter_polarimeter import *
except:
    print('Powermeter polarimeter not connected')
try:
    from polarimeter_rot_mount import *
except:
    print('Polarimeter rotational mount not connected')
try:
...
QWP_pola_wl_Delta = np.load(os.path.join(r'D:\Nonlinear_setup\Python_codes\polarimeter_calib','QWP_pola_wl_Delta_12Jul19_1634.npy'))
QWP_pola_wl_Delta_fx = sp.interpolate.interp1d(QWP_pola_wl_Delta[0],QWP_pola_wl_Delta[1],fill_value='cubic')

polarimeter_temporary_data = []

def polarimeter_measure_slow(wl=742,angs=np.arange(0,360,15),
                             ave_num=10,timesleep=0.1,
                             take_bg=False,use_sc_slider_to_take_bg=True,
                             live_plot=True,
                             plot=True,
                             is_calibrate=False,
                             verbose=True,
    ...
    pmp_wl(wl)
    if not is_calibrate:
        Delta = QWP_pola_wl_Delta_fx(wl)
    if live_plot:
        _fig = plt.figure()
        fig = _fig.add_subplot(111,projection='polar')
    
    if take_bg:
        if use_sc_slider_to_take_bg:
            block_sc()
            plt.pause(1)
            pmp_zero()
            plt.pause(1)
    ...
    for i,ang in enumerate(angs):
        set_pola_QWP_ang(ang)
        plt.pause(timesleep)
        curr_data = []
        for j in range(ave_num):
            curr_data.append(pmp_power())
    ...
    polarimeter_temporary_data.append(polarimeter_temporary_datum)
    ...
    if live_plot:
        ...
        if is_calibrate:
            ...
            if return_max_angle:
                ...
                polarimeter_fit = signal_rotQfixA(angle_range,alpha,gamma,QWP_pola_wl_Delta_fx(wl),1)
...
def polarimeter_measure(init_angle=-360,final_angle=350,timesleep=0.01): #to be fixed
    home_rot_pol()
    set_pola_QWP_ang(init_angle)
    global data
    data=[]
    
    def rotating_loop():
        set_pola_QWP_ang(final_angle)
    rotating_th = threading.Thread(target=rotating_loop)
    
    def acquire_data_loop():
        while rotating_th.isAlive():
            data.append(pmp_power())
...
def calib_polarimeter_QWP_Delta():
    ...
    for i,wl in enumerate(wls):
        ...
        curr_Delta = []
        for j in range(10):
            popt,perr = polarimeter_measure_slow(angs = np.arange(0,360,5), timesleep = 1e-6, ave_num = 10, take_bg=0, live_plot=0, is_calibrate=True)
    ...
    data_path = os.path.join(r'D:\Nonlinear_setup\Python_codes\polarimeter_calib','QWP_pola_wl_Delta_'+create_date_str()+'.npy')
