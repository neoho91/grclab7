...
SR830_ENBW_FACTOR_TC = [1./4,1./8,3./32,5./64]
SR830_NOISE_WAIT_TIME_FACTOR = [5.,7.,9.,10.]
SR830_SAMPLE_RATE = [6.25e-3,125e-3,250e-3,500e-3,1.,2.,4.,8.,16.,32.,64.,128.,256.,512.,-1]
SR830_SLOPE = [6.,12.,18.,24.]
...
def lockin_aux1in(): #get 1550 ref
    return float(srs830.ask("OAUX? 1"))

def lockin_aux3in(): #get sc ref
    return float(srs830.ask("OAUX? 3"))
...
def lockin_sample_rate(t=None):
    if t == None:
        idx = int(srs830.ask("SRAT?"))
        return SR830_SAMPLE_RATE[idx]
    else:
        idx = get_nearest_idx_from_list(t,SR830_SAMPLE_RATE)
        srs830.write("SRAT %i"%idx)
        return SR830_SAMPLE_RATE[idx]
...
def lockin_get_X_Xnoise_Y_Y_noise():
